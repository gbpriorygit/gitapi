...
<style>
    ...
    .card-header{background: linear-gradient(to top, #0f0c29, #0a2048, #24243e)!important;}
    .card-header1{background: linear-gradient(to bottom, #000000, #434343);}
    .card-header1 td {padding:15px 5px;}
    .custom_tr{padding:15px 5px;}
    .custom_tr td{    padding: 15px 5px!important;
        font-size: 21px;}
                ...
                <div class="card">
                  <div class="card-header row" data-background-color="blue">
                    ...
                    <div class="card-content taskc scrollable">
                      <table style="color:white !important;" class="table table-bordered table-responsive">
                        <?php
                          $output='<thead><td>Date</td><td>Lead</td><td>Changes</td></thead>';
...
<script type="text/javascript">
  ...
  if ( $('[name=lead]').val()!="%" || $page>0) {
    $('.filter_data').show();
  }

  $('.page-item').on('click',function(e) {//.pagination_btn
    e.preventDefault();
    if ( $('[name=lead]').val()!=lead) {
      $('.page_val').val(0);
    }
