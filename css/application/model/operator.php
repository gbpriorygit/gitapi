...
//////////////////////////////////////////////////////////////////
    ...
    public function getCallLog(){
        $sql="SELECT * FROM call_log  where user_id=:user_id order by calltime desc limit 100";
        $query=$this->db->prepare($sql);
    ...
    public function getUploads_test(){

        $sql="SELECT * FROM uploads WHERE `user_id`=:user_id AND DATE(`date`) = CURDATE()";
        $query=$this->db->prepare($sql);
        ...
        $sql="SELECT * FROM uploads WHERE `user_id`=:user_id AND uploads.seen=0 AND DATE(`date`) = CURDATE()";
        $query=$this->db->prepare($sql);
        $query->bindParam(':user_id',$_SESSION['user_id'],PDO::PARAM_INT);
        $query->execute();
    ...
    public function getUserShowFop(){
        $sql="SELECT show_fop FROM users WHERE user_id=:user_id";
        $query=$this->db->prepare($sql);
        $query->execute(array('user_id' =>$_SESSION['user_id']));
        $show_fopp = $query->fetch();
        return $show_fopp->show_fop;
    }
...
//}
    ...
    public function getFiles(){
        $sql="SELECT * FROM documents  where user_id=:user_id AND YEAR(date)=2020 order by date desc limit 100";
        $query=$this->db->prepare($sql);
        $query->execute(array('user_id' =>$_SESSION['user_id']));
        return $query->fetchAll();
    }

    public function getAllEmailsOperator(){
        $sql="SELECT * FROM emails where user_id=:user_id order by id desc";
        $query=$this->db->prepare($sql);
        $query->execute(array('user_id' =>$_SESSION['user_id']));
        return $query->fetchAll();
    }
    ...
    public function getNextLead($lead_id,$status,$direction){
        if ($status=="all") {
            $status="%";
        }
        if ($direction=="next") {
            $sql='SELECT lead_id FROM leads WHERE assigned_to=:user_id and lead_id<:lead_id and status like :status order by lead_id desc limit 1';
        }else{
            $sql='SELECT lead_id FROM leads WHERE assigned_to=:user_id and lead_id>:lead_id and status like :status limit 1';
        }
        $query = $this->db->prepare($sql);
        $query->execute(array(':lead_id'=>$lead_id,':user_id'=>$_SESSION['user_id'],':status'=>$status));
        $leadid=$query->fetch();
        print_r($leadid);
    ...
    public function resetAssignedTo($lead_id){
        $sql="UPDATE leads SET assigned_to=1 ,status=15 WHERE lead_id=:lead_id";
        $query=$this->db->prepare($sql);
    ...
    public function lead_send($phone_number,$assigned_to){
        ...
        if ($query->rowCount()>0) {
            ...
            $sql = "UPDATE leads_promo SET lead_send=1,lead_back=0  WHERE phone_number=:phone_number";
        ...
        {
            $sql = "INSERT INTO leads_promo(`phone_number`,`assigned_to`,`lead_send`)
...
///////////////////////////////////////////////////////////////////
    ...
    public function countByStatus($status_id='%'){
        $sql='SELECT count(lead_id) as c FROM leads where status like :status_id and assigned_to like :user_id';
        $query = $this->db->prepare($sql);
        $query->execute(array(':status_id'=>$status_id,':user_id'=>$_SESSION['user_id']));
        $c=$query->fetch();
        return $c->c;
    }
    ...
    public function getTasks($lead_id){
        $sql='SELECT tasks.*,users.first_name,users.last_name FROM tasks LEFT JOIN users ON users.user_id = tasks.user_id where tasks.lead_id=:lead_id and tasks.user_id=:user_id order by tasks.task_id desc limit 3';
        $query = $this->db->prepare($sql);
        $query->execute(array(':lead_id'=>$lead_id,':user_id'=>$_SESSION['user_id']));
        return $query->fetchAll();
    }
    ...
    public function getReminders($lead_id){
        $sql='SELECT reminders.*,users.first_name,users.last_name FROM reminders LEFT JOIN users ON users.user_id = reminders.user_id where reminders.lead_id=:lead_id and reminders.user_id=:user_id order by reminders.reminder_id desc limit 3';
        $query = $this->db->prepare($sql);
        $query->execute(array(':lead_id'=>$lead_id,':user_id'=>$_SESSION['user_id']));
        return $query->fetchAll();
    }
    ...
    //     $sql='SELECT notes.*,users.first_name,users.last_name FROM notes LEFT JOIN users ON users.user_id = notes.user_id where notes.lead_id=:lead_id and (notes.user_id=:user_id or users.role="admin") order by `note_id` desc';
    //     $query = $this->db->prepare($sql);
    //     $query->execute(array(':lead_id'=>$lead_id,':user_id'=>$_SESSION['user_id']));
    //     return $query->fetchAll();
    // }
    ...
    public function getEvolutions($lead_id){
        $sql='SELECT evolutions.*,users.first_name,users.last_name FROM evolutions LEFT JOIN users ON users.user_id = evolutions.user_id where evolutions.lead_id=:lead_id and (evolutions.user_id=:user_id or users.role="admin") order by `evolution_id` desc';
        $query = $this->db->prepare($sql);
        $query->execute(array(':lead_id'=>$lead_id,':user_id'=>$_SESSION['user_id']));
        return $query->fetchAll();
    }
    public function countBy($user_id='%',$status_id='%',$source_id='%'){
        //////////////////user////////////////////////
        if ($user_id!='%') {
            if (is_array($user_id)) {
                if ($user_id[0]!='%') {
        ...
        $sql='SELECT count(lead_id) as c FROM leads
                LEFT JOIN users  ON users.user_id = leads.assigned_to
                where (users.recycle =0 OR leads.assigned_to=0) and source '.$source_ids1.' and status '.$status_ids1.' and assigned_to '.$user_ids1;
        $query = $this->db->prepare($sql);
        $query->execute();
        $c=$query->fetch();
...
//////////////////////////////////////////////////////////////////////
    ...
    public function getAllTasks(){
        $sql='SELECT tasks.*,users.first_name,users.last_name FROM tasks LEFT JOIN users ON users.user_id = tasks.user_id where tasks.user_id=:user_id and  MONTH(`start`) = MONTH(CURRENT_DATE()) AND YEAR(`start`) = YEAR(CURRENT_DATE())  order by `task_id` desc limit 15';
        $query = $this->db->prepare($sql);
        $query->execute(array(':user_id'=>$_SESSION['user_id']));
        return $query->fetchAll();
    }
    ...
    public function getLastTasks($lead_id){
        $sql='SELECT * FROM tasks  where lead_id=:lead_id and user_id=:user_id  order by task_id desc limit 1';
        $query = $this->db->prepare($sql);
        $query->execute(array(':lead_id'=>$lead_id,':user_id'=>$_SESSION['user_id']));
        $start=$query->fetch();
    ...
    public function getChangelog($lead_id){
        $sql='SELECT * FROM log WHERE lead_id=:lead_id and user_id=:user_id ORDER BY id DESC';
        $query = $this->db->prepare($sql);
        $query->bindParam(':lead_id', $lead_id);
    ...
    public function getAllChangelog(){
        $sql='SELECT * FROM log WHERE user_id=:user_id ORDER BY id DESC limit 50';
        $query = $this->db->prepare($sql);
        $query->bindParam(':user_id',$_SESSION['user_id']);
        $query->execute();
        return $query->fetchAll(PDO::FETCH_ASSOC);
...
////////////////////////////////////////////////////
    ...
    public function getLeads($export=null){
        ...
        if (!isset($_REQUEST['search_'])) {//if filter
            ...
            $vars['assigned_to']    = $_SESSION['user_id'];
...
////////////////////////////////////////////////////////
        $assigned_to_ids1=" like ".$_SESSION['user_id'];
        $source_ids1=" like '%'";
        //
        // if ($vars['assigned_to']!='%') {
...
//////////////////////////////////////////////////////////
        ...
        if (!isset($_REQUEST['search_'])) {//if fitler
            $sql="SELECT leads.*,status.status_name,status.status_description,n.start as lasttask FROM leads
                        ...
                        AND assigned_to =".$_SESSION['user_id']."
    ...
    public function getLeads1(){
        ...
        if ($vars['orderby']=="status") {
            $vars['orderby']="status.status_name";
        }
        ...
        $sql="SELECT leads.*,status.status_name,status.status_description,n.start as lasttask FROM leads
                    ...
                    AND assigned_to like :assigned_to
                    AND .leads.status like :status
                    ORDER BY ".$vars['orderby']." ".$vars['orderdir']."
                    LIMIT :pager , :limiter";

        $sql_count="SELECT lead_id FROM leads WHERE
                            (
                              ((first_name LIKE :first_name AND last_name LIKE :last_name)
                                OR
                              (first_name LIKE :last_name AND last_name LIKE :first_name))
    ...
    public function createLead(){
        ...
        $sql = "INSERT INTO leads(`first_name`,`last_name`,`email`,`phone_number`,`assigned_to`)
                               VALUES(:first_name,:last_name,:email,:phone_number,:assigned_to)";
        ...
        $query->bindParam(':assigned_to', $_SESSION['user_id']);
    ...
    public function editLead($lead_id){
        ...
        $sql="UPDATE leads SET note=:note,status=:status,alt_number=:alt_number,data_sheets=:data_sheets WHERE lead_id=:lead_id";
        $query = $this->db->prepare($sql);
    ...
    public function uploadCV(){
        ...
        if (move_uploaded_file($_FILES["cv"]["tmp_name"],$target_file)) {
            $sql="INSERT INTO cv(user_id,url) VALUES(:user_id,:url)";
            $query=$this->db->prepare($sql);
        ...
        header("Location:".URL.$_SESSION['role'].'/links/cv');
    }
