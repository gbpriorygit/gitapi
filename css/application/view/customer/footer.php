                    ...
                    <td colspan="1" align="right">
                        <div class="btn-toolbar" style="margin: 0; vertical-align:middle">
                            <div id="divBtnCallGroup" class="btn-group">
                                <button id="btnCall" disabled class="btn btn-info" data-toggle="dropdown">Ready</button>
                            </div>&nbsp;&nbsp;
                            <div class="btn-group">
                            ...
                            <div id='divCallOptions' class='call-options btn-group' style='opacity: 0;'>
                        ...
                        <label  id="txtCallStatus"></label>&nbsp;&nbsp;<label style="padding-right: 10px;" id="txtRegStatus"></label>
...
<!--
    ...
    function notifyCustomer1MonthReminder(Customer1MonthReminder) {
        var audio = new Audio('<?php echo URL; ?>assets/beep.ogg');
        audio.play();
        var turl="<?=URL.$_SESSION['role']?>/viewLead/"+Customer1MonthReminder.lead_id+"?Customer1MonthReminder="+Customer1MonthReminder.Customer1MonthReminder_id;
        console.log(turl);
        pop(turl);
        var notification = new Notification('This client is called 1 month ago!', {
          icon: '<?php echo URL; ?>assets/img/meet.ico',
          body: Customer1MonthReminder.title
        });
        notification.onclick = function () {
         var turl="<?=URL.$_SESSION['role']?>/viewLead/"+Customer1MonthReminder.lead_id+"?Customer1MonthReminder="+Customer1MonthReminder.Customer1MonthReminder_id;
         console.log(turl);
...
/////////////////////////////////////////
    ...
    function getCustomer1MonthReminder(){
      $.ajax({
        url: '<?=URL?>api/getCustomer1MonthReminder',
        type: 'post',
        dataType:"json",
        success: function(data){
          $.each(data, function(i, item) {
               notifyCustomer1MonthReminder(data[i]);
          });
        },
        fail: function(data){
          console.log("error");
        ...
        complete:function(data){
          setTimeout(getCustomer1MonthReminder,9500);
        }
      });
    }

    function setCustomer1MonthReminderStatus(Customer1MonthReminderStatus_id){
      $.ajax({
        url: '<?=URL?>api/setCustomer1MonthReminderStatus/'+task_id,
        type: 'get',
        success: function(data){
    ...
    function countRecycle(){
      $.ajax({
        url: '<?=URL?>api/countRecycle/',
        type: 'get',
        dataType:'json',
        success: function(data){
          if (data.count){
            $('.recycleC').remove();
            $('.recycleNav >a >p').append('<font class="pull-right active recycleC">'+data.count+'</font>');
          }
        },
        fail: function(data){
          console.log("error");
