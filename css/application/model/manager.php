...
//////////////////////////////////////////////////////////////////
    ...
    public function getCallLog(){
      $sql="SELECT * FROM call_log LEFT JOIN users ON call_log.user_id = users.user_id WHERE users.role = 'operator' OR users.role = 'teamleader' order by calltime desc limit 200";
      $query=$this->db->prepare($sql);
      $query->execute();
    ...
    public function getUsers(){
        $sql="SELECT * FROM users WHERE users.role = 'operator' OR users.role = 'teamleader' OR users.role = 'result' order by first_name asc";
        $query=$this->db->prepare($sql);
        $query->execute();
        return $query->fetchAll();
    }
    ...
    public function editUser($user_id){
        ...
        $sql="UPDATE users SET username=:username,password=:password,first_name=:first_name,last_name=:last_name,role=:role,recycle=:recycle,supervisor=:supervisor WHERE user_id=:user_id";
        $query = $this->db->prepare($sql);
        $parameters=array(':username' => $_POST['username'],
                      ':password' => $_POST['password'],
                      ':first_name' => $_POST['first_name'],
...
////////////////////////////////////////////////////
    ...
    public function getActivity(){
        ...
        $sql='SELECT * FROM log LEFT JOIN users ON log.user_id = users.user_id WHERE users.role = "operator" OR users.role = "teamleader" ORDER BY id DESC  LIMIT :pager , :limiter';
        $query = $this->db->prepare($sql);
        $query->bindParam(':pager', $vars['pager'], PDO::PARAM_INT);
        $query->bindParam(':limiter', $vars['limiter'], PDO::PARAM_INT);
        $query->execute();

        $sql='SELECT * FROM log LEFT JOIN users ON log.user_id = users.user_id WHERE users.supervisor = '.$_SESSION['user_id'].' ORDER BY id DESC ';
    ...
    public function getManagerLeads($export=null){
