...
//////////////////////////////////////////////////////////////////
    ...
    public function getCallLog(){
        $sql="SELECT * FROM call_log LEFT JOIN users ON call_log.user_id = users.user_id WHERE users.supervisor = ".$_SESSION['user_id']." order by calltime desc limit 200";
        $query=$this->db->prepare($sql);
        $query->execute();
    ...
    public function getUsers(){
        $sql="SELECT * FROM users WHERE users.supervisor = ".$_SESSION['user_id']." order by first_name asc";
        $query=$this->db->prepare($sql);
        $query->execute();
        return $query->fetchAll();
    }
...
///////////////////////////////////////////////////////////////////
    ...
    public function countBy($user_id='%',$status_id='%',$source_id='%',$assigned_from='%'){
        ...
        $sql='SELECT count(lead_id) as c FROM leads
                LEFT JOIN users  ON users.user_id = leads.assigned_to
                WHERE users.supervisor = '.$_SESSION['user_id'].' AND source '.$source_ids1.' AND status '.$status_ids1.' AND assigned_to '.$user_ids1.' AND assigned_from '.$assigned_froms1;///
        //print_r($sql);
        $query = $this->db->prepare($sql);
        $query->execute();
        $c=$query->fetch();
...
////////////////////////////////////////////////////
    ...
    public function getActivity(){
        ...
        $sql='SELECT * FROM log LEFT JOIN users ON log.user_id = users.user_id WHERE users.supervisor = '.$_SESSION['user_id'].' ORDER BY id DESC  LIMIT :pager , :limiter';
        $query = $this->db->prepare($sql);
...
//////////////////////////////////////////////////////////
        ...
        if (!isset($_REQUEST['search_'])) {//if fitler
            $sql="SELECT leads.*,status.status_name,status.status_description,n.start as lasttask FROM leads
                        ...
                        AND note like '%".$note."%'
                        AND users.supervisor like ".$_SESSION['user_id']."
            ...
            $sql="SELECT leads.*,status.status_name,status.status_description,n.start as lasttask FROM leads
                      ...
                      AND (users.recycle =0 OR leads.assigned_to=0)
                      AND users.supervisor = ".$_SESSION['user_id']."
                      AND list_id like :list_id
                      ORDER BY ".$vars['orderby']." ".$vars['orderdir']."
                      LIMIT :pager , :limiter";
            //AND assigned_to ".$assigned_to_ids1."
...
//////////////////////////////////////////////////////////
        ...
        if (!isset($_REQUEST['search_'])) {//if fitler
            $sql="SELECT leads.*,status.status_name,status.status_description FROM leads
                        LEFT JOIN users  ON (users.user_id = leads.assigned_to AND users.supervisor = ".$_SESSION['user_id'].") 
                        LEFT JOIN status ON status.status_id = leads.status
                        WHERE
                        (
                          ((leads.first_name LIKE :first_name AND .leads.last_name LIKE :last_name)
    ...
    public function getRecycle($export=null){
        ...
        if (!isset($_REQUEST['search_'])) {//if fitler
            $sql="SELECT leads.*,status.status_name,status.status_description,n.start as lasttask FROM leads
                            LEFT JOIN users  ON users.user_id = leads.assigned_to AND users.supervisor = 12
                            LEFT JOIN status ON status.status_id = leads.status
                            LEFT JOIN reminders AS n ON n.reminder_id =
                                 ( SELECT noq.reminder_id
                                   FROM reminders AS noq
