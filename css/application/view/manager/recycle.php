...
<style>
    body{
        background-image:linear-gradient(rgba(0,0,0,0.3),rgba(0,0,0,0.5)),  url("<?php echo URL; ?>assets/img/bg_recycle2.jpg");
                        ...
                        <table class="table table-hover table-leads">
                            ...
                            <th><strong>
                                ...
                                <select data-placeholder=" " class="cho" multiple name="status[]" id="status">
                                    ...
                                    <!-- <option value='%'>&nbsp;</option> -->
                                    <!-- | <?php //echo $this->model->countByR($assigned_to,$status,$source);?> -->
                                    <?php
                                    $output='';
                                    foreach ($statuses as $status1) {
                                        $output.='<option value="'.$status1->status_id.'" >'.$status1->status_name.' | '.$this->model->countByR($assigned_to,$status1->status_id,$source).'</option>';//| '.$this->model->countByR($assigned_to,$status1->status_id,$source).'
                                    }
                                    echo $output;
                                    ?>
                                </select>
                            ...
                            foreach ($leads as $lead) {
                                ...
                                $output.=$lead->status_name==="R-New"
                                    ...
                                    $lead->status_name==="Mid"
                                        ?
                                        '<td> <button  class="btn btn-success btn-sm" style="background: linear-gradient(to right, #67b26f, #4ca2cd); min-width:93px;">' .$lead->status_name.'</button></td>'
                                        :(
                                ...
                                if (isset($operator1)) {
                                    ...
                                    $output.=$lead->assigned_to ==24
                                        ?
                                        '<td> <button class="btn btn-red btn-sm" style="background: linear-gradient(to right, #000000, #434343); min-width:93px;">'.$operator1->first_name.' '.$operator1->last_name.'</button></td>'
                                        :(
                                        $lead->assigned_to ==26
                                            ?
                                            '<td> <button  class="btn btn-success btn-sm" style="background: linear-gradient(to right, #136a8a, #267871); min-width:93px;">'.$operator1->first_name.' '.$operator1->last_name.'</button></td>'
                                            :(
                                        $lead->assigned_to ==27
                                            ?
                                            ...
                                            '<td> <button  class="btn btn-success btn-sm" style="background: linear-gradient(to right, #eb5757, #000000); min-width:93px;">'.$operator1->first_name.' '.$operator1->last_name.'</button></td>'
...
<script type="text/javascript">
    ...
    $(function() {
        $('.reset_btn').on('click',function() {
            document.location.href="<?=URL.$_SESSION['role'].'/recycle';?>";
        });
        <?php
    ...
    function quickViewLead(lead_id){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                // document.getElementById("quickEmail").value=leadData.email;
                ...
                document.getElementById("quickMental").textContent=leadData[0].mental;
                var leadUrl='/dashboard/admin/viewLead/'+leadData[0].lead_id;
                document.getElementById("quickViewUrl").setAttribute('href', leadUrl);
